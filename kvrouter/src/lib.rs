...
impl<T: LoadBalancer> OverloadHandler<T> {
    ...
    async fn next(&mut self, key: &[u8]) -> String {
    ...
    pub async fn run(&mut self) {
        while let Some(msg) = self.rx.recv().await {
            ...
            match msg {
                Msg::Next(key, sx) => {
                    let backend: String = self.next(&key).await;
...
pub async fn set_backends_handler(
    State(state): State<Communicator>,
    Json(SetBackends { hosts }): Json<SetBackends>,
) -> impl IntoResponse {
    let _ = state.sender.send(Msg::SetBackends(hosts)).await;
    StatusCode::OK
}
